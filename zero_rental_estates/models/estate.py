...
class Estate(models.Model):
    ...
    reference = fields.Char(string="Reference of Estate")
    rental_price = fields.Float(string="price of the rental (without charges)", default=0.0, digits=dp.get_precision('price of the rental'))
    street = fields.Char()
    description = fields.Text(string="Description")
    area = fields.Integer(string="Area")
    postale_code = fields.Char(string="Postal code")
    city = fields.Many2one('lb.city')
    district = fields.Many2one('lb.district')
    country = fields.Many2one('res.country', string="Countries", default=_get_default_country)
    ...
    ('reference_unique',
    'UNIQUE(reference)',
    "the reference of estate must be unique"),
    ]
